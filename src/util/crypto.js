...
export function b64(bytes) {
    const str64 = typeof (bytes) === "string" ? global.btoa(bytes) : global.btoa(String.fromCharCode.apply(null, bytes));
    return str64.replace(/\//g, "_").replace(/\+/g, "-").replace(/=/g, "");
}

export function clearB64(str64) {
    return str64.replace(/\//g, "_").replace(/\+/g, "-").replace(/=/g, "").replace('\n', '');
}
