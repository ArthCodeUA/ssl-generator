...
export class LetsEncrypt {
  ...
  async registerAccount() {
    ...
    this.account.registration_sig = hex2b64(
      (
        await openssl(
    ...
    await axios
      ...
      .then(async (response) => {
        ...
        await this.getNonce().then((nonce) => {
          ...
          this.account.update_protected_b64 = b64(
            JSON.stringify(this.account.update_protected_json)
          );
        });
      });
  }
  ...
  async updateAccount() {
    ...
    this.account.update_sig = hex2b64(
      (
        await openssl(
          `openssl dgst -sha256 -hex -sign account.key ${config.SIGNATURE_REQUEST_PATH}`,
          config.OPENSSL_DIR
    ...
    await axios
      ...
      .then(async () => {
        await this.getNonce().then((nonce) => {
          ...
          this.order.order_protected_b64 = b64(
            JSON.stringify(this.order.order_protected_json)
          );
        });
      });
  }
  ...
  async createNewOrder() {
    ...
    await axios
      ...
      .then((response) => {
        ...
        for (
          let i = 0;
          i < this.order.order_response.authorizations.length;
          i++
        ) {
  ...
  async prepareChallenge(n) {
    ...
    await this.getNonce().then(async (nonce) => {
      ...
      this.authorizations[auth_url].auth_sig = hex2b64(
        (
          await openssl(
            `openssl dgst -sha256 -hex -sign account.key ${config.SIGNATURE_REQUEST_PATH}`,
            config.OPENSSL_DIR
  ...
  async requestChallenge(n) {
    ...
    await axios
      ...
      .then((response) => {
        ...
        const challenges =
          this.authorizations[auth_url].auth_response.challenges;
        for (let i = 0; i < challenges.length; i++) {
          let challenge_dict = challenges[i];

          // HTTP challenge
          if (challenge_dict["type"] === "http-01") {
            this.authorizations[auth_url].file_challenge_uri =
              challenge_dict["url"];
            this.authorizations[auth_url].file_challenge_object =
              challenge_dict;
          }

          // DNS challenge
          if (challenge_dict["type"] === "dns-01") {
            this.authorizations[auth_url].dns_challenge_uri =
              challenge_dict["url"];
  ...
  async confirmChallenge(n, method) {
    ...
    await this.getNonce().then(async (nonce) => {
      ...
      const protected_b64 = b64(JSON.stringify(protected_json));
      this.authorizations[auth_url][method + "_protected_json"] =
        protected_json;
      ...
      this.authorizations[auth_url][method + "_challenge_sig"] = hex2b64(
        (
          await openssl(
            `openssl dgst -sha256 -hex -sign account.key ${config.SIGNATURE_REQUEST_PATH}`,
            config.OPENSSL_DIR
  ...
  async validateChallenge(n, method) {
    const auth_url = this.order.order_response.authorizations[n];
    const challenge_url =
      this.authorizations[auth_url].dns_challenge_object.url;
    await axios
      .post(
        challenge_url,
        {
      ...
      .then(async (response) => {
        this.authorizations[auth_url][method + "_challenge_response"] =
          response.data;
    ...
    await axios
      ...
      .then(async (response) => {
        ...
        if (response.data.status === "pending") {
          console.log(
            `Checked, but status still pending, recheck in ${
              +config.RECHECK_INTERVAL / 1000
            } seconds`
          );
          setTimeout(async () => {
            await this.reCheckSignature(n);
  ...
  async reCheckSignature(n) {
    ...
    await this.getNonce().then(async (nonce) => {
      ...
      const protected_b64 = b64(JSON.stringify(protected_json));
      this.authorizations[auth_url].recheck_auth_protected_json =
        protected_json;
      ...
      this.authorizations[auth_url].recheck_auth_sig = hex2b64(
        (
          await openssl(
            `openssl dgst -sha256 -hex -sign account.key ${config.SIGNATURE_REQUEST_PATH}`,
            config.OPENSSL_DIR
  ...
  async prepareFinalizeOrder() {
    await this.getNonce().then(async (nonce) => {
      ...
      this.order.finalize_protected_b64 = b64(
        JSON.stringify(this.order.finalize_protected_json)
      );
      fs.writeFileSync(
        config.SIGNATURE_REQUEST_PATH,
        `${this.order.finalize_protected_b64}.${this.order.finalize_payload_b64}`
      );
      this.order.finalize_sig = hex2b64(
        (
          await openssl(
            `openssl dgst -sha256 -hex -sign account.key ${config.SIGNATURE_REQUEST_PATH}`,
            config.OPENSSL_DIR
      ...
      .then(async (response) => {
        ...
        if (
          ...
          await this.getNonce().then(async (nonce) => {
            ...
            this.order.cert_protected_b64 = b64(
              JSON.stringify(this.order.cert_protected_json)
            );
            fs.writeFileSync(
              config.SIGNATURE_REQUEST_PATH,
              `${this.order.cert_protected_b64}.${this.order.cert_payload_b64}`
            );
            this.order.cert_sig = hex2b64(
              (
                await openssl(
                  `openssl dgst -sha256 -hex -sign account.key ${config.SIGNATURE_REQUEST_PATH}`,
                  config.OPENSSL_DIR
  ...
  async reCheckOrderSignature() {
    await this.getNonce().then(async (nonce) => {
      ...
      this.order.recheck_order_protected_b64 = b64(
        JSON.stringify(this.order.recheck_order_protected_json)
      );
      fs.writeFileSync(
        config.SIGNATURE_REQUEST_PATH,
        `${this.order.recheck_order_protected_b64}.${this.order.recheck_order_payload_b64}`
      );
      this.order.recheck_order_sig = hex2b64(
        (
          await openssl(
            `openssl dgst -sha256 -hex -sign account.key ${config.SIGNATURE_REQUEST_PATH}`,
            config.OPENSSL_DIR
